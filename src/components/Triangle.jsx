        ...
        return (
          <g key={edge.id}>
            ...
            <rect
              ...
              stroke={axisMeta.color} strokeOpacity={0.7}
              style={{ fill: 'var(--svg-circle-fill)', cursor: 'pointer', filter: 'drop-shadow(0 1px 2px rgba(15,23,42,0.06))' }}
              onClick={() => onSelectAxis?.(edge.id)}
            />
        ...
        return (
          <g key={v.id} style={{ cursor: 'pointer' }} onClick={() => onSelectVertex?.(v.id)}>
            <circle
              ...
              style={{ fill: 'var(--svg-circle-fill)', transition: 'all 200ms', filter: isSelected ? 'url(#glow)' : 'drop-shadow(0 2px 4px rgba(15,23,42,0.08))' }}
            />
