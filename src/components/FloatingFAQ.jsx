  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full bg-brand-teal text-background shadow-lg hover:bg-brand-teal-light transition flex items-center justify-center"
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-background-elevated">
              <h2 className="text-lg font-semibold flex items-center gap-2"><HelpCircle size={20} className="text-brand-teal-light" /> Foire aux questions</h2>
              <button onClick={() => setOpen(false)} aria-label="Fermer" className="text-text-muted hover:text-text"><X size={20} /></button>
            </div>
            <ul className="overflow-y-auto divide-y divide-background-elevated">
              {[...FAQ_ENTRIES].sort((a, b) => a.ordre - b.ordre).map((f) => {
                ...
                return (
                  <li key={f.id}>
                    ...
                    >
                      <ChevronDown size={16} className={`shrink-0 mt-1 text-text-muted transition ${isOpen ? 'rotate-180 text-brand-teal-light' : ''}`} />
                      <div className="flex-1">
                        <p className="text-sm font-semibold mb-1">{f.question}</p>
