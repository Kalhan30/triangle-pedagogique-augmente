    ...
    <section className="max-w-6xl mx-auto px-4 md:px-6 py-6 grid md:grid-cols-[360px_1fr] gap-6 animate-fade">
      <div className="space-y-3">
        <div className="card p-4">
          <h3 className="text-sm font-semibold mb-2 flex items-center gap-2"><Sparkles size={16} className="text-brand-teal-primary" /> Proposez votre séance</h3>
          ...
          <textarea
            ...
            className="w-full bg-background text-text border border-border-subtle rounded-lg p-2 text-sm focus:outline-none focus:border-brand-teal"
            aria-label="Décrivez votre situation pédagogique"
          />
          {warning && (
            <p className="text-xs mt-2 p-2 rounded bg-semantic-warning/10 text-brand-amber-primary flex items-start gap-2">
          ...
          <button
            ...
            className="mt-3 w-full py-2 rounded-lg bg-brand-teal-primary text-white font-semibold text-sm hover:bg-brand-teal-text disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center justify-center gap-2"
          >
          ...
          {scenarios.map((s) => (
            <button
              ...
              className={`card p-3 w-full text-left transition hover:border-brand-teal ${
                selected?.id === s.id ? 'border-brand-teal-primary bg-brand-teal-light' : ''
              }`}
            >
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-3 mt-5">
        <div className="p-3 rounded-lg" style={{ background: 'rgba(20, 184, 166, 0.1)', borderLeft: '3px solid #14B8A6' }}>
          <h4 className="text-xs font-bold uppercase tracking-wider mb-2 text-brand-teal-primary">Rôle de l'IA</h4>
        ...
        <div className="p-3 rounded-lg" style={{ background: 'rgba(139, 92, 246, 0.1)', borderLeft: '3px solid #8B5CF6' }}>
          <h4 className="text-xs font-bold uppercase tracking-wider mb-2 text-brand-violet-primary">Rôle humain</h4>
      ...
      {scenario.pointsAttention && scenario.pointsAttention.length > 0 && (
        <div className="mt-3 p-3 rounded-lg" style={{ background: 'rgba(245, 158, 11, 0.1)', borderLeft: '3px solid #F59E0B' }}>
          <h4 className="text-xs font-bold uppercase tracking-wider mb-2 text-brand-amber-primary">Points d'attention</h4>
