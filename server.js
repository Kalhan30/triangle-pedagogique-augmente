...
app.post('/api/generate-scenario', async (req, res) => {
  ...
  if (!process.env.MISTRAL_API_KEY || process.env.MISTRAL_API_KEY === 'votre_cle_api_mistral_ici') {
    return res.status(500).json({ 
      ...
      message: 'Veuillez configurer MISTRAL_API_KEY dans le fichier .env' 
    });
  }

  // Contexte réglementaire par niveau
  const levelContexts = {
...
Génère une réponse au format JSON strict avec la structure suivante :
{
  ...
  "axes": {
    "ens-sav": 0.5, // Poids de l'axe Enseignant-Savoir (préparation) entre 0.0 et 1.0
    "ens-elev": 0.5, // Poids de l'axe Enseignant-Elève (relation) entre 0.0 et 1.0
    "elev-sav": 0.5 // Poids de l'axe Elève-Savoir (apprentissage) entre 0.0 et 1.0
  }
}
